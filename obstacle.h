...
void obstacleInit(Obstacle& obs) {
	...
	if (choice == 2) { obs.sprite.setPosition(second); }
	if (choice == 3) { obs.sprite.setPosition(third); }
	if (choice == 4) { obs.sprite.setPosition(fourth); }
...
void obstacleUpdate(Obstacle& obs){
	...
	if (obs.sprite.getPosition().y >= 800.f) {
